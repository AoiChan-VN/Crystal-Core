...
// [!] Code: Global Batch Auto Save Manager (Mode B)
public class AutoSaveManager {
    ...
    public void scheduleSave() {
        ...
        if (plugin.getConfig().getBoolean("auto-save.console-log", true)) {
            Bukkit.getConsoleSender().sendMessage("§e[Aoi Save] Saving data in " + delay + " seconds...");
        }

        autoSaveTask = Bukkit.getScheduler().runTaskLaterAsynchronously(plugin, () -> {

            Set<UUID> dirty = dirtyTracker.getDirtyPlayers();
            ...
            storageManager.saveBatch(dirty);

            if (plugin.getConfig().getBoolean("auto-save.console-log", true)) {
                Bukkit.getConsoleSender().sendMessage("§a[Aoi Save] Data saved successfully.");
            }
